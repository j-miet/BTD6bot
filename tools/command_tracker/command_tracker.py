...
def keyboard_tracker(key_pressed: Key | KeyCode | None) -> None:
    ...
    if isinstance(key_pressed, KeyCode):
        ...
        if key_pressed.char == "+" and TrackerVals.ct_user_input == "":
            ...
            TrackerVals.coordinates_file.write("elif round == " + str(TrackerVals.ct_round_counter) + ":\n")
...
def begin_round() -> int:
    ...
    while True:
        try:
            start = input(
                "Please input starting round (1-100).\n"
    ...
    match cmd[0]:
        ...
        case "-ucp":
            if len(cmd[1:]) < 2:
                ...
                return
            TrackerVals.coordinates_file.write(
                "    " + cmd[1] + ".upgrade(" + cmd[2] + ", cpos_x=" + x + ", cpos_y=" + y + ")\n"
            )
...
def run_tracker() -> None:
    ...
    print("-------------------------")
    print("Press right mouse to add commands text, '+' to change round, and F8 (or CTRL+C) to quit.")
    ...
    while True:
        ...
        if TrackerVals.ct_user_input == "<>":  # right mouse button identifier
            ...
            comment_input = input(
                "Insert command text --->"
            )  # this input() is now part of main thread so it won't cause lag like it would if it were inside mouse_tracker
