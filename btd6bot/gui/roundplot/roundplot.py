...
def append_rounds(
    plan: list[str], first_r: int, first_r_end: int, round_labels: list[str]
...
def plot(round_labels: list[str], rounds: list[list[str]], plan_name: str) -> None:
    ...
    if sys.platform == "win32":  # don't bother with window icon on non-windows os
    ...
    try:
        ...
        fig.suptitle(
            f'Round commands and round times of current plan "{plan_name}"\n'
            r"Time data is updated after each successful run -> last updated: "
            + r"$\bf{%s}$" % update_date
    ...
    ax_rounds.set_title(
        "Round commands - " "Order of execution is top to bottom. Rounds with no commands are not included.",
        fontsize="medium",
    )
    ...
    round_slider = Slider(
        axis_position, "Round slider", slider_min_val, slider_max_val, valinit=0, valstep=1, initcolor="none"
    )
