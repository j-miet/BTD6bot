...
class MainWindow:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.maps_box = MainWindowCombobox(
            mainwindow=self, master=mainframe, state="readonly", values=self._get_maps(), name="mapbox"
        )
        ...
        self.strat_box = MainWindowCombobox(mainwindow=self, master=mainframe, state="readonly", name="stratbox")
        ...
        self.MAPSCREEN_TEXT_STR = (
            r" ___  _____  ___    __  _          _    " + "\n"
            r"| _ )|_   _||   \  / / | |__  ___ | |_  " + "\n"
            r"| _ \  | |  | |) |/ _ \| '_ \/ _ \|  _| " + "\n"
        ...
        self.help_button = tk.Button(
            mainframe, text="Help", height=2, width=5, command=self._help_window, padx=20, pady=5
        )
        self.help_button.grid(column=2, row=0, padx=30, pady=10, sticky="nwe")

        self.settings_button = tk.Button(
            mainframe, text="Settings", height=2, width=5, command=self._settings_window, padx=20, pady=5
        )
        self.settings_button.grid(column=3, row=0, pady=10, sticky="nwe")

        self.hotkey_button = tk.Button(
            mainframe, text="Set\nhotkeys", height=2, width=5, command=self._hotkey_window, padx=20, pady=5
        )
        self.hotkey_button.grid(column=3, row=1, pady=10, sticky="we")

        self.queueoptions_button = tk.Button(
            mainframe, text="Queue mode\nmaplist", height=2, width=5, command=self._queue_mode_window, padx=20, pady=5
        )
        ...
        self.start_button = tk.Button(
            mainframe, text="Initialize bot", height=2, command=self._monitoring_window, state="active", padx=10, pady=0
        )
    ...
    def _monitoring_window(self) -> None:
        ...
        if not self.reader_init:
            ...
            def adjust_ocr_init_label_to_root() -> None:
                ...
                geom = step2.split("+")
                adj_geom = [
                    str(val) for val in (int(geom[0]) - 150, int(geom[1]) - 400, int(geom[2]) + 75, int(geom[3]) + 220)
                ]
            ...
            tk.Label(
                ocr_init_window, text="Loading ocr model into memory, please wait.", font=("Times", 16, "bold")
            ).pack()
        ...
        monitoringwindow = MonitoringWindow(
            self._choose_mode(), self.replay.get(), self.queue.get(), self.collection.get(), self.farming.get()
        )
    ...
    def _queue_mode_window(self) -> None:
        ...
        queuewindowthread = threading.Thread(target=self._is_queue_mode_screen, args=[queuewindow.get_queuewindow()])
    ...
    def _hotkey_window(self) -> None:
        ...
        hotkeywindowthread = threading.Thread(target=self._is_hotkeywindow, args=[hotkeywindow.get_hotkeywindow()])
    ...
    def _help_window(self) -> None:
        ...
        helpwindowthread = threading.Thread(target=self._is_helpwindow, args=[helpwindow.get_helpwindow()])
    ...
    def _settings_window(self) -> None:
        ...
        settingsthread = threading.Thread(target=self._is_settingswindow, args=[settingswindow.get_settingswindow()])
