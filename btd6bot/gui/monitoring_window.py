...
class MonitoringWindow:
    ...
    def __init__(self, all_plans_list: list[str], replay: str, queue: str, collection: str, farming: str) -> None:
        ...
        self.roundtime_label = tk.Label(
            self.monitoringwindow, width=15, height=3, text="Round timer", relief="sunken", font=os_font
        )
        ...
        self.roundtime_display = tk.Label(
            self.monitoringwindow, width=8, height=3, relief="sunken", textvariable=self.roundtime, font=os_font
        )
        ...
        if guivars_adjust:
            try:
                ...
                self.monitor_mapscreen = ttk.Label(
                    self.monitor_mapscreen_frame, image=photo, compound="top", anchor="nw", justify="left"
                )
            ...
            try:
                photo = tk.PhotoImage(
                    file=gui_paths.MAP_IMAGES_PATH / (plan_data.return_map(self.all_plans[0]) + ".png")
                )
                self.monitor_mapscreen = ttk.Label(
                    self.monitor_mapscreen_frame, image=photo, compound="top", anchor="nw", justify="left"
                )
        ...
        self.monitor_run_button = tk.Button(
            self.monitoringwindow, text="Run", command=self._stop_or_run, state="active", padx=10, pady=10
        )
        self.monitor_run_button.grid(column=5, row=4, sticky="ne")

        if self.farming == "On":
            self.runs_label = tk.Label(
                self.monitoringwindow, width=15, height=3, text="Runs completed", relief="sunken", font=os_font
            )
            self.runs_label.grid(column=1, row=4, sticky="sw", padx=10, pady=(10, 0))

            self.runs = tk.IntVar(value=0)
            self.runs_display = tk.Label(
                self.monitoringwindow, width=8, height=3, relief="sunken", textvariable=self.runs, font=os_font
            )
            self.runs_display.grid(column=1, row=4, sticky="s", padx=(75, 0), pady=(10, 0))

            monitor_farming_text = tk.Label(
                self.monitoringwindow, text="Farm mode: On", relief="sunken", padx=10, pady=10
            )
            ...
            monitor_queuemode_text = tk.Label(
                self.monitoringwindow, text="Queue mode: " + str(self.queue_val), relief="sunken", padx=10, pady=10
            )
            monitor_queuemode_text.grid(column=5, row=2, sticky="se")

            monitor_replay_text = tk.Label(
                self.monitoringwindow, text="Replay mode: " + str(self.replay_val), relief="sunken", padx=10, pady=10
            )
        ...
        while self.current_plans != []:
            ...
            if BotData.victory:
                if retries == 1:
                    ...
                    self.plans_status.append(f"{self.current_plans[0]} -- success [{attempt_number}/{retries}]")
            else:
    ...
    def _run_bot(self) -> None:
        ...
        if customres:
            cprint(
                "[Custom Resolution] " + str(resolution_val[0]) + "x" + str(resolution_val[1]) + "\n"
        ...
        if self.farming == "On":
            ...
            while True:
                ...
                try:
                    ...
                    self.monitor_mapscreen = ttk.Label(
                        self.monitor_mapscreen_frame, image=new_image, compound="top", anchor="nw", justify="left"
                    )
                    self.monitor_mapscreen.image = new_image  # type: ignore[attr-defined]
                    self.monitor_mapscreen.place(relx=0.5, rely=0.5, anchor="center")
                except tk.TclError:
