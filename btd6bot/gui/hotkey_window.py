...
class HotkeyWindow:
    ...
    def __init__(self) -> None:
        ...
        self.sethotkeybutton = tk.Button(
            self.hotkeywindow, text="Set hotkey", width=15, height=1, font=os_font, command=self._set_hotkey
        )
        ...
        self.info = tk.Label(
            self.hotkeywindow, justify="left", relief="sunken", text=self._showhotkeyhelp(), font=os_font
        )
    ...
    def _set_hotkey(self) -> None:
        ...
        if new_input == () and new_input2 == ():
            ...
            selected = self.hotkeyoptionlist.curselection()[0]
            self.input_key = pynput.keyboard.Listener(
                on_press=lambda event: self._press_hotkey(key=event, selected_row=selected)
            )
        elif new_input2 != ():
            selected = self.guihotkeyoptionlist.curselection()[0]
            self.input_key = pynput.keyboard.Listener(
                on_press=lambda event: self._press_guihotkey(key=event, selected_row=selected)
            )
