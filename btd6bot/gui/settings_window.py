...
class SettingsWindow:
    ...
    def __init__(self) -> None:
        ...
        self.settings_window.geometry('580x450+100+550')
        self.settings_window.minsize(580,450)
        self.settings_window.maxsize(580,450)
        ...
        record_toggle.grid(column=0, row=2, columnspan=2, sticky='nw')
        ...
        debug_text = tk.Label(self.settings_window, 
                              text='o-------------------------------------o\n'
                                   ...
                                   'o-------------------------------------o')
        debug_text.grid(column=0, columnspan=2, row=4, sticky='w', padx=5, pady=(15, 1))
        ...
        time_limit_label.grid(column=0, row=5, sticky='se', padx=15, pady=(5,1))
        time_limit_current_value = tk.Label(self.settings_window, textvariable=self.time_limit)
        time_limit_current_value.grid(column=1, row=5, sticky='sw', padx=1, pady=(5,1))

        self.time_limit_entry = tk.Entry(self.settings_window, width=10)
        self.time_limit_entry.grid(column=0, row=6, sticky='sw', pady=(1,10), padx=21)

        time_limit_button = tk.Button(self.settings_window, text="Set time limit", anchor='w', padx=5,
                                           command=self.set_time_limit_value)
        time_limit_button.grid(column=1, row=6, sticky='w', pady=(1,10))

        time_limit_text = tk.Text(self.settings_window, wrap=tk.WORD, width=62, height=3)
        time_limit_text.grid(column=0, row=7, columnspan=5)
        time_limit_text.insert('end', "Time limit, in seconds, until bot stops trying to place or upgrade a monkey and "
                               "returns to menu. Only needed if ocr gets stuck; high value (120 or greater) is "
                               "recommended.")
