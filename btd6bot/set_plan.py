...
def _update_time_data(
    plan_name: str, recorded_timedata: list[str], full_timedata: Any, version: int
    ...
    data_dict = {"update_date": date, "version": version, "roundtimes": roundstimes_dict, "time_total": time_total}
...
def farming_print() -> None:
    ...
    cprint(
        "Collection event farming mode enabled.\n"
...
def get_rounds(difficulty: str, gamemode: str) -> tuple[int, int]:
    ...
    if difficulty == "EASY":
        match gamemode:
            case "STANDARD" | "PRIMARY" | "SANDBOX":  # sandbox is only for quick single round testing
                return 1, 40
            case "DEFLATION":
                return 31, 60
            case _:
...
def plan_run(plan_name: str, plan_module: ModuleType, data: tuple[str, str, str, int, int, str, bool]) -> None:
    ...
    try:
        ...
        if (
            not data[-1]
            and _check_if_temp_valid(_maindata.maindata["times_temp"])
            and _maindata.debug_get_mode() == _maindata.Debug.NONE
        ):
