...
class Hero(Monkey):  # type: ignore[misc]
    ...
    def __init__(self, pos_x: float, pos_y: float) -> None:
        ...
        self._debug_level_hero()

    def _debug_level_hero(self) -> None:
        if _maindata.debug_get_mode() == _maindata.Debug.SETUP_STATIC_STATE:
            level: int = _maindata.maindata["debug"]["hero_startlevel"]
            if isinstance(level, int) and level >= 2 and level <= 20:
                kb_mouse.click((self._pos_x, self._pos_y), shifted=True)
                time.sleep(0.2)

                # to level up N from 1, click upgrade button N-1 times
                if self._panel_pos == "left":
                    kb_mouse.click(get_click("ingame", "hero_leftpanel_upgradelevel"), level - 1, shifted=True)
                elif self._panel_pos == "right":
                    kb_mouse.click(get_click("ingame", "hero_rightpanel_upgradelevel"), level - 1, shifted=True)
            else:
                return
            ...
            kb_mouse.press_esc()
            time.sleep(0.25)
            _maindata.maindata["debug"]["hero_startlevel"] = -1  # reset back to default
