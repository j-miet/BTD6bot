...
class Rounds:
    ...
    def return_menu(final_round_start: float, total_start: float, final_round: int) -> None:
        ...
        if _maindata.debug_get_mode() == _maindata.Debug.POS_STATIC:
            dprint("Position testing finished, returning to main menu...")
            timing.counter(3)
            kb_mouse.press_esc()
            time.sleep(1)
            kb_mouse.click(get_click("ingame", "home_button2"))
                ...
                if not weak_substring_check(letter, get_text("ingame", "upgrade_text"), OCR_READER):
                    if times.current_time() - start_time > Rounds.ENTERMAP_TIMELIMIT:
                        for _ in range(3):
                            kb_mouse.press_esc()
                        raise BotError(
                            "Failed to enter game: wrong map name in plan file, or map/game mode not " "unlocked.", 1
                        )
                else:
                    loop = False
                    break
    ...
    def round_check(prev_round: int, map_start: float, mode: str = "") -> int:
        ...
        if current_round == Rounds.begin_round:
            ...
            return current_round
        elif _maindata.debug_get_mode() == _maindata.Debug.POS_STATIC:
            dprint(f"Round {current_round}:")
        elif (
            _maindata.debug_get_mode() == _maindata.Debug.SETUP_STATIC_STATE
            and current_round <= _maindata.debug_get_startround()
        ):
            ...
            if _maindata.maindata["internal"]["skip_roundcheck"]:
                ...
                while True:
                    round_value = strong_substring_check(
                        str(current_round) + "/" + str(Rounds.end_round),
        ...
        BotData.set_data(
            round_time=Rounds.current_round_begin_time,
